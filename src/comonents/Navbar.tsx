  ...
  return (
    <div className="navbar bg-base-100 shadow-sm px-4 ">
      <div className="navbar-start">
        <Link
          href={state.isAuthenticated ? "/dashboard" : "/"}
          className="btn btn-ghost text-2xl rounded-md"
        >
          Kanban Board
        </Link>
      </div>
      ...
      <div className="navbar-end hidden sm:flex items-center">
        ...
        {!state.isAuthenticated && (
          <>
            <button
              className="btn btn-outline btn-primary mx-2 text-lg"
              onClick={() => dispatch({ type: ModalActionEnum.OPEN_LOGIN })}
            >
              Sign In
            </button>
            <button
              className="btn btn-outline btn-secondary mx-2 text-lg"
          ...
          {isOpen && (
            <div className="absolute right-0 mt-3 w-48 bg-base-100 shadow-lg rounded-lg p-4 z-50">
              <div className="w-full mb-2 py-2">
                <label className="swap swap-rotate -translate-x-1/2 left-1/2 cursor-pointer">
                  <input type="checkbox" onClick={toggleTheme} />
                  <svg
                    className="swap-off h-10 w-10 fill-current"
                    xmlns="http://www.w3.org/2000/svg"
              ...
              <button
                ...
                onClick={() =>
                  dispatch({ type: ModalActionEnum.OPEN_REGISTER })
                }
              >
                Sign Up
              </button>
